...
// ═══════════════════════════════════════════════════════════════
// AIRDROP AI — Production Build
// Real QVAC SDK, File Intelligence + Personal Knowledge Base
// ═══════════════════════════════════════════════════════════════
...
const extractTextFromFile = async (uri, mimeType, name) => {
  try {
    ...
    // For PDFs and other binary formats, extract what we can
    const raw = await FileSystem.readAsStringAsync(uri, { encoding: FileSystem.EncodingType.Base64 });
    // Return a placeholder with file metadata for non-text files
...
export default function App() {
  ...
  useEffect(() => {
    ...
    bootModels();
    return () => {
      if (llmModelId) QVAC.unloadModel({ modelId: llmModelId }).catch(() => {});
      if (embedModelId) QVAC.unloadModel({ modelId: embedModelId }).catch(() => {});
    };
  }, []);
  ...
  const askAI = async () => {
    ...
    try {
      ...
      const systemPrompt = context
        ? `You are a private AI assistant running entirely on this device. Answer using the provided context. Be direct and concise.\n\nContext:\n${context}`
        : `You are a private AI assistant running entirely on this device. Answer concisely and accurately.`;
    ...
    return (
      <View style={styles.bootScreen}>
        ...
        <View style={styles.bootLogo}>
          ...
          <Text style={styles.bootTitle}>AirdropAI</Text>
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.header}>
        <View>
          <Text style={styles.headerTitle}>◈ AirdropAI</Text>
